...
def cmd_check(args: Dict[str, Any]) -> int:
    ...
    # dependency-graph recovery is best-effort, so a clean result may be partial.
    producer = (model.creator or '').lower()
    if 'esp-idf-sbom' not in producer and 'esp-idf sbom' not in producer:
        log.warn('Checking an SBOM not generated by esp-idf-sbom; the scan results may be incomplete.')

    record_list: List[Dict[str, str]] = []
    exit_code = 0

    # Without --check-all-packages, scan only the packages reachable from the
    # root through depends_on -- the application's dependency graph.
    if args['check_all_packages']:
        packages = model.packages
    else:
        packages = sbom.app_packages(model)
        ...
        if args['local_db']:
            ...
            for pkg in packages:
                ...
                if args['extended_scan']:
                    # Include the CPE product names and the package's cve-keywords.
                    keywords += [cpe.split(':')[4] for cpe in pkg.cpes]
    ...
    # The project package is the DESCRIBES root; fall back to the first scanned
    # package, and tolerate an empty/rootless SBOM (a malformed or third-party
    # input) instead of crashing with an IndexError.
    project_pkg = next((p for p in packages if p.ref == model.root), packages[0] if packages else None)
    proj_name = project_pkg.package_name if project_pkg else ''
    proj_ver = project_pkg.version if project_pkg else ''
...
def cmd_license(args: Dict[str, Any]) -> int:
    # Force the build options the license report needs: file_tags so licenses
    # and copyrights are collected from the component/package files, and the
    # config-only / unused components removed so the report covers only what is
    ...
    # Follow the SBOM's depends_on edges from the root to get every package used
    # by the application, then collect their licenses and copyrights.
    used = sbom.app_packages(model)
    summary = sbom.summarize_licenses(used, args['unify_copyrights'])
    license_concluded = summary.license_concluded
    licenses = summary.licenses
    copyrights = summary.copyrights

    packages: List[Dict[str, Any]] = []
    if args['packages']:
        for package in used:
            package_summary = sbom.summarize_licenses([package], args['unify_copyrights'])
            packages.append(
                {
                    'name': package.name,
                    'license_concluded': package_summary.license_concluded,
                    'licenses': package_summary.licenses,
                    'copyrights': package_summary.copyrights,
                }
            )
    ...
    try:
        ...
        with log.progress(
            ...
            for manifest in manifests:
                ...
                tags = sbom.SBOMDirTags(manifest['_dst'], exclude_dirs)
...
@click.option(
    ...
    help=(
        ...
        'markdown - Print report in Markdown format.'
    ),
)
@click.pass_context
